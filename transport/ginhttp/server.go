...
func (server *Server) Kind() string {
	return operator.Ternary(server.options.TLSConf == nil, "http", "https")
}
...
func (server *Server) Metadata() map[string]string {
	return server.options.MetaData
}
...
func (server *Server) runRegistrar(ctx context.Context) error {
	...
	go func() {
		...
		err := server.options.Registrar.Register(ctx, server)
	...
	case <-ctx.Done():
		...
		return errors.Join(server.options.Registrar.Deregister(ctx, server), <-registerErrC)
	}
}
