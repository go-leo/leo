package lgrpc

import (
	"context"
	"fmt"
	"net"
	"sort"
	"strconv"
	"strings"
	"time"

	"google.golang.org/grpc/attributes"
	"google.golang.org/grpc/resolver"

	"codeup.aliyun.com/qimao/leo/leo/registry"
)

var _ resolver.Builder = new(ResolverBuilder)

type ResolverBuilder struct {
	discovery registry.Discovery
	scheme    string
}

func NewResolverBuilder(discovery registry.Discovery, scheme string) resolver.Builder {
	return &ResolverBuilder{discovery: discovery, scheme: scheme}
}

// Build 创建Resolver并启动服务名解析
// target格式：
// Scheme://Authority/Endpoint
// consul://username:password@ip:port/service.name?scheme=http&datacenter=dev&token=12345&wait_time=1s&tls=true
// nacos://ip1:port1,ip2:port2/service.name?contentType=yaml&namespace=ns&group=g&dataId=d
func (rb *ResolverBuilder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	// 比较target的Scheme是否与discovery的Scheme一直
	if target.URL.Scheme != rb.Scheme() {
		return nil, fmt.Errorf("target schema is %s, but discovery schema is %s", target.URL.Scheme, rb.Scheme())
	}
	// 从url上获取serviceInfo信息
	uri := target.URL
	instance := registry.Builder().
		Name(strings.TrimLeft(uri.Path, "/")).
		Scheme("grpc").
		Build()
	// 创建Resolver
	r := &Resolver{
		closeC:    make(chan struct{}),
		cc:        cc,
		opts:      opts,
		discovery: rb.discovery,
		instance:  instance,
	}
	// 开始解析
	if err := r.Start(); err != nil {
		return nil, err
	}
	return r, nil
}

func (rb *ResolverBuilder) Scheme() string {
	return rb.scheme
}

var _ resolver.Resolver = new(Resolver)

type Resolver struct {
	closeC chan struct{}
	// cc 和 opts 原生gRPC框架传入的，可以控制客户端连接的状态和行为
	cc   resolver.ClientConn
	opts resolver.BuildOptions
	// discovery Leo服务发现组件
	discovery registry.Discovery
	// serviceInfo 当前需要被解析的服务
	instance registry.ServiceInstance
}

func (r *Resolver) ResolveNow(o resolver.ResolveNowOptions) {}

func (r *Resolver) Close() {
	close(r.closeC)
}

func (r *Resolver) Start() error {
	ctx := context.Background()
	getInstanceCtx, cancelFunc := context.WithTimeout(ctx, 1*time.Second)
	defer cancelFunc()
	// 一开始先获取一次服务信息
	service, err := r.discovery.GetInstances(getInstanceCtx, r.instance)
	if err != nil {
		return err
	}
	// 更新grpc的连接状态和ip地址
	if err := r.update(service); err != nil {
		return err
	}
	// 监听服务变化
	watcher, err := r.discovery.Watch(ctx, r.instance)
	if err != nil {
		return err
	}
	go func() {
		defer watcher.Close(ctx)
		instanceC := make(chan []registry.ServiceInstance)
		watcher.Notify(instanceC)
		defer watcher.StopNotify(instanceC)
		for {
			select {
			case <-r.closeC:
				// 退出
				return
			case service := <-instanceC:
				// 有事件产生，更新grpc的连接状态和ip地址
				if err := r.update(service); err != nil {
					fmt.Println(err)
				}
			}
		}
	}()
	return nil
}

func (r *Resolver) update(service []registry.ServiceInstance) error {
	if len(service) <= 0 {
		return nil
	}
	// registry.ServiceInfo转成resolver.Address
	addresses := r.convertService(service)
	// 更新grpc连接状态
	return r.cc.UpdateState(resolver.State{Addresses: addresses})
}

func (r *Resolver) convertService(service []registry.ServiceInstance) []resolver.Address {
	addresses := make([]resolver.Address, 0, len(service))
	for _, service := range service {
		attr := &attributes.Attributes{}
		address := resolver.Address{
			Addr:       net.JoinHostPort(service.IP(), strconv.Itoa(service.Port())),
			ServerName: service.Name(),
			Attributes: attr,
		}
		addresses = append(addresses, address)
	}
	sort.Slice(addresses, func(i, j int) bool {
		return strings.Compare(addresses[i].Addr, addresses[j].Addr) > 0
	})
	return addresses
}
