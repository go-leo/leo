...
func TestBuilderWrite2(t *testing.T) {
	...
	for _, tt := range []struct {
		name string
		fn   func(b *stringx.Builder) (int, error)
		n    int
		want string
	}{
		{
			"Write",
			func(b *stringx.Builder) (int, error) { return b.Write([]byte(s0)) },
			len(s0),
			s0,
		},
		{
			"WriteRune",
			func(b *stringx.Builder) (int, error) { return b.WriteRune('a') },
			1,
			"a",
		},
		{
			"WriteRuneWide",
			func(b *stringx.Builder) (int, error) { return b.WriteRune('世') },
			3,
			"世",
		},
		{
			"WriteString",
			func(b *stringx.Builder) (int, error) { return b.WriteString(s0) },
			len(s0),
			s0,
		},
	} {
...
func TestBuilderAllocs(t *testing.T) {
	...
	// stringx.Builder to escape and be heap allocated.
	...
	if n != 1 {
		t.Errorf("stringx.Builder allocs = %v; want 1", n)
	}
}
