...
func (r *Registrar) deregister(service *registry.ServiceInfo) error {
	...
	groupName := r.extractString(metadata, _GroupName)     // 分组名称
	port := uint64(service.Port)                           // 服务实例port
	ip := service.Host                                     // 服务实例IP
...
type Discovery struct {
	...
	logger log.Logger
	sync.RWMutex
	subscribeCallbackMap map[string]func(services []model.Instance, err error)
...
func NewDiscovery(cli naming_client.INamingClient, logger log.Logger) *Discovery {
	d := &Discovery{
		...
		subscribeCallbackMap: make(map[string]func(services []model.Instance, err error)),
...
func (d *Discovery) watch(service *registry.ServiceInfo) (<-chan []*registry.ServiceInfo, error) {
	...
	d.subscribeCallbackMap[serviceKey] = func(services []model.Instance, err error) {
		if err != nil {
			d.logger.Error(err)
		}
...
func (factory *RegistrarFactory) Create() (registry.Registrar, error) {
	client, err := NewNacosNamingClient(factory.URI)
	if err != nil {
		return nil, err
	}
