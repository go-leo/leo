...
import (
	...
	"bytes"
	"fmt"
	"github.com/go-leo/gox/bytex/diffx"
	...
	for _, file := range files {
		t.Run(filepath.Base(file), func(t *testing.T) {
			...
			if !bytes.Equal(stdoutBuf, want["stdout"].Data) {
				t.Errorf("wrong stdout: %s", diffx.Diff("want", want["stdout"].Data, "have", stdoutBuf))
			}
			if !bytes.Equal(stderrBuf, want["stderr"].Data) {
				t.Errorf("wrong stderr: %s", diffx.Diff("want", want["stderr"].Data, "have", stderrBuf))
			}
			delete(want, "stdout")
			delete(want, "stderr")
			...
			err = filepath.WalkDir(out, func(name string, info fs.DirEntry, err error) error {
				...
				if !bytes.Equal(data, f.Data) {
					t.Errorf("wrong %s: %s", short, diffx.Diff("want", f.Data, "have", data))
				}
				return nil
			})
			if err != nil {
